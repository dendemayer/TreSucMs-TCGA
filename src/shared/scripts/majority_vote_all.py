...
def apply_project(row):
    for project in projects:
        if re.search(f'\/{project}\/', row):
            return project


final_DF['project'] = final_DF['file_path'].apply(apply_project)
final_DF['cutoff'] = final_DF['file_path'].apply(lambda x: re.search('cutoff_\d{1,2}', x).group(0))
...
if len(projects) > 1 and len(pipelines) > 1:
    ...
    pdfs_to_merge = [pdf_aggr, venn_project, venn_pipeline]
