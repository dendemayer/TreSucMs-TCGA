...
if len(projects) > 1 and len(pipelines) > 1:
    ...
    gene_list_multi = temp_DF_3.loc[:, 'gene_name'].value_counts().to_frame().query('gene_name > 1').reset_index().loc[:, 'index'].tolist()
    gene_list_single = temp_DF_3.loc[:, 'gene_name'].value_counts().to_frame().query('gene_name == 1').reset_index().loc[:, 'index'].tolist()
    temp_DF_3 = pd.concat([temp_DF_3.replace(False, pd.NA).set_index('gene_name').T.loc[:, gene].fillna(method='ffill', axis=1).fillna(method='bfill', axis=1) for gene in gene_list_multi], axis=1).T.reset_index().drop_duplicates()
