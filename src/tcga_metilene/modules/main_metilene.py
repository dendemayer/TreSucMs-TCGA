...
def entry_fct(OUTPUT_PATH, PROJECT, DRUGS, Snakemake_all_files, cutoff, threshold, cores):
    ...
    summary_tables = create_summary_table.return_summary_tables(
        OUTPUT_PATH, PROJECT, DRUGS, cutoff, threshold)

    Snakemake_all_files = Snakemake_all_files + summary_tables
    breakpoint()
    snakemake.snakemake(snakefile=Snakefile, targets=Snakemake_all_files, workdir=shared_workdir, cores=cores, forceall=False, force_incomplete=True, dryrun=True, debug=True)
    breakpoint()
