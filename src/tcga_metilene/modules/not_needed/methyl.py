...
def create_table_for_metilene(OUTPUT_PATH, PROJECT, DRUGS, DRUGS_title,
    ...
    def create_dropped_ref_sorted(summary_DF, complement):
        gender_list = []
        ...
        # Chromosome  Start   alive;aa10d6-...;carboplatin...;female;TCGA-HNSC
        ...
        for gender in gender_list:
            ...
            if gender == 'both':
                ...
                dropped_file_name = 'dropped_REF_sorted.tsv'
                temp_REF_DF.to_csv(os.path.join(
                    path_gen, dropped_file_name), sep='\t', index=False)
                ...
                summary_name = \
                    'summary_dead_alive_dropped_NA_dropped_REF_sorted.tsv'
                if complement:
                    summary_name = ('summary_dead_alive_' +
                                    'dropped_NA_dropped_REF_' +
                                    'sorted_complement.tsv')
                temp_DF.to_csv(os.path.join(path_gen, summary_name), sep='\t',
                               index=False)
                logger.info(
            ...
            # or write the gender limited parts:
                ...
                # gender)].to_csv(os.path.join(path_gen, 'summary_DF'),
                # sep='\t')
                ...
                if complement:
                    summary_name = \
                        ('summary_dead_alive_dropped_' +
                         'NA_dropped_REF_sorted_complement.tsv')
                temp_DF.to_csv(os.path.join(
                    path_gen, summary_name), sep='\t', index=False)
                logger.info('create_table_for_metilene_6:\t{}'.format(
                            os.path.join(
                                PROJECT_title, DRUGS_title, gender,
                                summary_name)))
                logger.info('create_table_for_metilene_6:\t{}'.format(
                            os.path.join(
                                PROJECT_title,
                                DRUGS_title, gender, dropped_file_name)))
        # TODO: how to handle NA values?
    create_dropped_ref_sorted(summary_DF, False)
    create_dropped_ref_sorted(summary_DF_complement, True)
...
# ################# START multiproject part
    ...
    for gender in gender_list:
        if not os.path.exists(os.path.join(OUTPUT_PATH, PROJECT_title,
            ...
            try:
                intersect_DF = pd.read_csv(os.path.join(met_dir,
                                                        'intersect_out.tsv'),
                                           header=None, sep='\t')
            except Exception as e:
                print(f'Exception thrown: {e}')
                file_path = os.path.join(OUTPUT_PATH, PROJECT_title,
    ...
    for gender in gender_list:
        if not os.path.exists(os.path.join(OUTPUT_PATH, PROJECT_title,
            ...
            try:
                ...
                print(f'throwing exception:\n{e}')
                print('continuing to next gender')
                continue
            ...
            # ### BEGIN check both vital states per project #####
            # the plot shall include a dead and a alive graph for every
            # project, check if both vital states are available for all
            # projects:
            project_to_delete = []
            ...
            if len(project_to_delete) != 0:
                ...
                for element in project_to_delete:
                    project_set.remove(element)
            # ### END check both vital states per project #####
            ...
            for i in list_min_max:
                ...
                if intersect_DF[filt].empty:
                    ...
                    for start in intersect_DFi.index:
                        # start greater than range start and smaller than range
                        # end?
                    ...
                    # filter out the projects which do not habe both vital
                    # states in them:
                    for project in project_to_delete:
                        filt = DF_candidate_all_project[
                            'project'] == project_to_delete[0]
                        DF_candidate_all_project = DF_candidate_all_project[
                            ~filt]
                    try:
...
def create_table_all_projects(OUTPUT_PATH, DRUGS, PROJECT_DRUG_UUID,
    ...
    # ################## BEGIN complement
    # ### copy and concatenate the complement from every project per gender
    # into PROJECTS_combined/
    # gender/
    ...
    for gender in ['female', 'male', 'both']:
        summary_file = ('summary_dead_alive_dropped_NA_' +
                        'dropped_REF_sorted_complement.tsv')
        first = True
        for project in projects:
            summary_path = os.path.join(
                OUTPUT_PATH, project, DRUGS_title, gender, summary_file)
            if os.path.exists(summary_path):
                if first:
                    temp_DF = pd.read_csv(summary_path, sep='\t',
                                          index_col=['Chromosome', 'Start'])
                    first = False
                else:
                    temp_DF = temp_DF.join(pd.read_csv(summary_path, sep='\t',
                                                       index_col=['Chromosome',
                                                                  'Start']))
        os.makedirs(os.path.join(OUTPUT_PATH, PROJECT_title, DRUGS_title,
                                 gender), exist_ok=True)
        temp_DF.to_csv(os.path.join(OUTPUT_PATH, PROJECT_title, DRUGS_title,
                                    gender, summary_file), sep='\t')
...
def create_snake_config(OUTPUT_PATH, PROJECT_title, DRUGS_title, PROJECT_list,
    ...
    # dir:
    name_copied = os.path.join(SCRIPT_PATH, os.path.pardir, 'Snakes',
                               'snakemake_config_' + PROJECT_title + '_' +
                               DRUGS_title + '.yaml')
